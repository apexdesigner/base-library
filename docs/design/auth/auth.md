...
The [avatar](components/avatar.component.md) component displays the current user's identity in the toolbar with a menu for logout and user switching. The [manage role assignments](components/manage-role-assignments.component.md) component provides a UI for assigning roles to users. The [select user field](components/select-user-field.component.md) and [select role field](components/select-role-field.component.md) components are schema form fields for picking users and roles by id. The [select role name field](components/select-role-name-field.component.md) component is a schema form field for picking a role by name.

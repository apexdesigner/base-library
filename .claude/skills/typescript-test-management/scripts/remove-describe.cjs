...
function usage() {
  console.error('Usage: node remove-describe.js <file-path> <describe-name>');
  console.error('Note: Supports partial matching of describe name');
  process.exit(1);
}
...
function parseArgs() {
  ...
  return { filePath, describeName };
}

function findDescribeByName(sourceFile, name, exactMatch = false) {
  const matches = [];
...
function main() {
  const { filePath, describeName } = parseArgs();
  ...
  try {
    ...
    let matches = findDescribeByName(sourceFile, describeName, true);

    // If no exact match, try partial match
    if (matches.length === 0) {
      matches = findDescribeByName(sourceFile, describeName, false);
    }

    if (matches.length === 0) {
      console.error(`Error: No describe block found matching "${describeName}"`);
      process.exit(1);
    }

    if (matches.length > 1) {
      console.error(`Error: Multiple describe blocks match "${describeName}":`);
    ...
    console.log(`Removed describe "${match.name}" (lines ${startLine}-${endLine})`);
  } catch (error) {

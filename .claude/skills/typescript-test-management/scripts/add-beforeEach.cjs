...
function usage() {
  console.error('Usage: node add-beforeEach.cjs <file-path> [options]');
  console.error('');
  ...
  console.error('  --async               Create async beforeEach');
  console.error('');
  console.error('Examples:');
  console.error('  # Simple inline body');
  console.error('  node add-beforeEach.cjs file.ts --body="jest.clearAllMocks()"');
  console.error('');
  console.error('  # Async beforeEach with body from file');
  console.error('  node add-beforeEach.cjs file.ts --describe="Suite" --async --body-file=/tmp/setup.js');
  process.exit(1);
}
...
function findExistingBeforeEach(describeBody) {
  let found = null;
...
function createBeforeEachBlock(indent, body, isAsync) {
  const asyncKeyword = isAsync ? 'async ' : '';
  const indentedBody = indentCode(body, indent + indent);

  return `${indent}beforeEach(${asyncKeyword}() => {
...
function main() {
  ...
  try {
    ...
    const existingBeforeEach = findExistingBeforeEach(describeBody);
    if (existingBeforeEach) {
      console.error('Error: beforeEach already exists in this describe block. Use replace-test-body.cjs to modify it.');
      process.exit(1);
    }

    // Insert at the beginning of the describe body (after opening brace)
    let insertPosition;
    if (describeBody.isKind(SyntaxKind.Block)) {
    ...
    const beforeEachBlock = createBeforeEachBlock(indent, body, isAsync);
    const textToInsert = `\n${beforeEachBlock}\n`;
    ...
    // Find the added beforeEach
    let addedBeforeEach = null;
    updatedFile.forEachDescendant((node) => {
      if (addedBeforeEach) return;
      if (node.isKind(SyntaxKind.CallExpression)) {
        const expr = node.getExpression();
        if (expr.getText() === 'beforeEach') {
          addedBeforeEach = node;
        }
      }
    });

    if (addedBeforeEach) {
      const startLine = addedBeforeEach.getStartLineNumber();
      const endLine = addedBeforeEach.getEndLineNumber();
      console.log(`Added beforeEach at lines ${startLine}-${endLine}`);
    } else {
      console.log('Added beforeEach');
    }
  } catch (error) {
